...
if ( !empty($_POST)) { // if $_POST filled then process the form
	...
	if ($valid) { // if valid user input update the database
		if($fileSize > 0) { // if file was updated, update all fields
			...
			$sql = "UPDATE fr_assignments set htmlspecialchars(assign_per_id) = ?, htmlspecialchars(assign_event_id) = ?, filename = ?,filesize = ?,filetype = ?,filecontent = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($person,$event,$fileName,$fileSize,$fileType,$content,$id));
			Database::disconnect();
			...
			$pdo = Database::connect();
			$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$sql = "UPDATE fr_assignments set htmlspecialchars(assign_per_id) = ?, htmlspecialchars(assign_event_id) = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
