...
if ( !empty($_POST)) { // if not first time through
	...
	if ($valid) 
	{
		...
		$sql = "INSERT INTO fr_persons (htmlspecialchars(fname),htmlspecialchars(lname),htmlspecialchars(email),htmlspecialchars(mobile),htmlspecialchars(password),htmlspecialchars(title),
		filename,filesize,filetype,filecontent,htmlspecialchars(mailingaddress),htmlspecialchars(city),htmlspecialchars(state),htmlspecialchars(zip)) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		$q = $pdo->prepare($sql);
		$q->execute(array($fname,$lname,$email,$mobile,$passwordhash,$title,
		$fileName,$fileSize,$fileType,$content,$mailingaddress,$city,$state,$zip));
