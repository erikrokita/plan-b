...
if ( !empty($_POST)) { // if $_POST filled then process the form
	...
	if (empty($mailingaddress)) {
		$mailingaddressError = 'Please enter a Mailing Address';
		$valid = false;
	}
	if (empty($city)) {
	...
	if ($valid) { // if valid user input update the database
		...
		if($fileSize > 0) { // if file was updated, update all fields
			...
			$sql = "UPDATE fr_persons  set htmlspecialchars(fname) = ?, htmlspecialchars(lname) = ?, htmlspecialchars(email) = ?, htmlspecialchars(mobile) = ?, htmlspecialchars(password) = ?, htmlspecialchars(title) = ?, filename = ?,filesize = ?,filetype = ?,filecontent = ?,htmlspecialchars(mailingaddress) = ?,htmlspecialchars(city) = ?,htmlspecialchars(state) = ?,htmlspecialchars(zip) = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($fname, $lname, $email, $mobile, $password, $title, $fileName,$fileSize,$fileType,$content,$mailingaddress,$city,$state,$zip, $id));
			Database::disconnect();
			...
			$pdo = Database::connect();
			$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$sql = "UPDATE fr_persons  set htmlspecialchars(fname) = ?, htmlspecialchars(lname) = ?, htmlspecialchars(email) = ?, htmlspecialchars(mobile) = ?, htmlspecialchars(password) = ?, htmlspecialchars(title) = ?,htmlspecialchars(mailingaddress) = ?,htmlspecialchars(city) = ?,htmlspecialchars(state) = ?,htmlspecialchars(zip) = ? WHERE id = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array($fname, $lname, $email, $mobile, $password, $title,$mailingaddress,$city,$state,$zip,  $id));
			Database::disconnect();
			header("Location: fr_persons.php");
		}
	}
